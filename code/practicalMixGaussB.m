...
%The goal of this file is to generate some data from a one-dimensional
...
for (cData =1:nData)
    ...
    data(:,cData) = randn(1)*sqrt(mixGauss.cov(h))+mixGauss.mean(h);
end;
...
for (cIter = 1:nIter)
   ...
   for (cData = 1:nData)
        ...
        numeratorOne = getGaussProb(data(cData),mixGaussEst.mean(1),mixGaussEst.cov(1))*mixGaussEst.weight(1);
        numeratorTwo = getGaussProb(data(cData),mixGaussEst.mean(2),mixGaussEst.cov(2))*mixGaussEst.weight(2);
        denominator = numeratorOne + numeratorTwo;
        responsibilities(1,cData) = numeratorOne/denominator;
        responsibilities(2,cData) = numeratorTwo/denominator;
   end;
   ...
   for (cGauss = 1:k) 
        ...
        mixGaussEst.weight(cGauss) = (sum(responsibilities(cGauss,:)))/((sum(responsibilities(1,:)))+(sum(responsibilities(2,:)))); 
        ...
        mixGaussEst.cov(1,1,cGauss) = (sum(responsibilities(cGauss,:).*(data-mixGaussEst.mean(cGauss)).*(data-mixGaussEst.mean(cGauss))))/(sum(responsibilities(cGauss,:)));
   end;
...
for(cData = 1:nData)
    ...
    %Gaussians model.
    like = getGaussProb(thisData,mixGaussEst.mean(1),mixGaussEst.cov(1))*mixGaussEst.weight(1)+getGaussProb(thisData,mixGaussEst.mean(2),mixGaussEst.cov(2))*mixGaussEst.weight(2);
...
for(cData = 1:nData)
    ...
    boundValue = (thisQ(1)*log((getGaussProb(thisData,mixGaussEst.mean(1),mixGaussEst.cov(1)))*mixGaussEst.weight(1)/thisQ(1)))+(thisQ(2)*log((getGaussProb(thisData,mixGaussEst.mean(2),mixGaussEst.cov(2)))*mixGaussEst.weight(2)/thisQ(2)));
