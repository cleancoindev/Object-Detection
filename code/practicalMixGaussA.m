...
%The aim of Part D is to apply these techniques to real data.
...
for (cY = 1:imY); 
    ...
    for (cX = 1:imX);          
        ...
        posteriorSkin(cY,cX) = (priorSkin * likeSkin) / ((priorSkin * likeSkin)+(priorNonSkin * likeNonSkin));
    end;
end;
...
a = 1/((sqrt(det(gaussCov)))*((2*pi)^(nDim/2)));
...
for i=1:nData
    covData = covData + (data(:,i)-meanData)*(data(:,i)-meanData)';
end
